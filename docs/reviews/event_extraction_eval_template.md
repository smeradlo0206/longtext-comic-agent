...
summary text, the single most salient event across selected chunks, and the
shortest exact supporting quote.
...
- Did the model return exactly one `EventProposalV1`?
- Did the summary describe one event from the selected chunks?
...
- Did `quote_matched` and `char_range_matched` pass?
