...
exchange, one normal action, or sharing a place are not relationship signals. "甲救下乙" may be a
...
exists. ClaimProposalV1 records an attributable assertion: "甲说乙是叛徒" is normally a Claim.
...
source_speaker and never promote the statement to narrated truth or canonical fact. "甲说乙背叛了甲"
may be a stated BETRAYS signal, but "甲说乙是叛徒" does not prove 乙 betrayed 甲.
...
"甲怀疑乙是叛徒" is SUSPECTS, not a Relationship Signal. StateChangeProposalV1 records
...
ordering. no longer trusts means DISTRUSTS plus FORMATION: for "甲从此不再信任乙", use
relationship_domain=TRUST, relationship_kind=DISTRUSTS, signal_effect=FORMATION, and the explicit
"从此" anchor. TRUST is the neutral domain name and does not mean positive trust. Relationship
...
mention_text must appear verbatim in a selected SourceChunk. Do not expand "他", "她", "那人", or
"师兄" into an outside name. A context event is only local event context and cannot add a
...
def _reject_traitor_label_as_betrayal(signal: RelationshipSignalProposalV1) -> None:
    ...
    if str(signal.relationship_kind) != "BETRAYS":
        return
    if not any(
        evidence.quote_text is not None and "背叛" in evidence.quote_text
        for evidence in signal.evidence_refs
    ):
        raise ValueError(
