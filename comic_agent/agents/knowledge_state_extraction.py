...
or expression mechanism: “林舟怀疑……” -> SUSPECTS + UNKNOWN, “林舟坚信……” ->
BELIEVES + UNKNOWN, and “林舟不知道……” -> UNAWARE + UNKNOWN. Use STATED only when
the character explicitly expresses that state in speech or writing, for example “我不信……”
...
status word such as 怀疑、坚信、觉得 or 不知道. A narrator stating that a character
“觉得……不安全” is a source-supported SUSPECTS + UNKNOWN state, not an empty Batch.
...
- EVENT: only a concrete occurrence, discovery, change, or action. Concrete verbs such as
  ...
  describe the occurrence itself (for example, "妹妹已经离开小镇").
...
HEARD does not imply target_kind=CLAIM. For example, "城门已经封锁" is WORLD_FACT + HEARD
when the target is the world state; only "掌柜声称城门已经封锁" is CLAIM when the target is
the shopkeeper's statement. "守卫故意隐瞒了山路的位置" is WORLD_FACT, and "山中有鬼" is
...
"山中有鬼", never DISBELIEVES + CLAIM + "山中有鬼的传言".
...
Normalize a rumor's content target from "山中有鬼的传言" to "山中有鬼". Normalize a statement
about a world state from "掌柜说城门已经封锁" to "城门已经封锁" unless the statement itself is
the target, in which case use "掌柜声称城门已经封锁" and CLAIM. Do not output both an atomic
