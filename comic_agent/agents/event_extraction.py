...
If the text contains several events, choose the single most salient event that
can be directly supported by one short quote.
...
summary must only state facts directly supported by evidence_refs[0].quote_text.
Every actor, action, object, and outcome mentioned in summary must be visible or
directly inferable from the quote.
...
event_type should name the single event, not a merged category.
